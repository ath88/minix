...
)
{
        ...
        if ((r = attach_memory((key_t) m_ptr->PROS_BUFFER1, (key_t) m_ptr->PROS_BUFFER2, (key_t) m_ptr->PROS_BITMAP)) != OK)
...
)
{
    ...
    if ((shmid2 = shmget(key2, IPC_CREAT, 0666)) < 0)
        return ENOMEM;
    if ((shmid3 = shmget(key3, IPC_CREAT, 0666)) < 0)
        return ENOMEM;
